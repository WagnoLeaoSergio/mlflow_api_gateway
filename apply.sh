...
original_name="mlflow_api_gateway"
original_urlname="mlflow_api_gateway"
original_description="Awesome mlflow_api_gateway created by WagnoLeaoSergio" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/mlflow_api_gateway "${name}"
...
then
    ...
    echo "then run 'mlflow_api_gateway' or 'python -m mlflow_api_gateway'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'mlflow_api_gateway' or 'python -m mlflow_api_gateway' or 'poetry run mlflow_api_gateway'"
fi
